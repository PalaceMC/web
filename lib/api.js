    ...
    try {
        ...
        // Refresh token
            ...
            if (req.requestTime < creation + 30_000) {
                ...
                ipTokens[req.remoteExpanded] = null
                console.error(`Warning! Unexpected REFRESH request from [ ${req.remoteExpanded} ] !!`)
                return doError(res, 429)
            }

            // Ensure we haven't made this request too late, but nicely
            if (req.requestTime >= creation + 60_000) {
                console.error(`Warning! Late REFRESH request from [ ${req.remoteExpanded} ] !!`)
                return doError(res, 400, "Token Expired")
            }
...
api.post('/discord/webhook', async (req, res) => {
    ...
    res.json(await discord.webhookSend(type, name, avatar, message))
})
