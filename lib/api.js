...
function validateToken(req, res) {
    ...
    // null is okay, tell server it needs to exchange
    if (ipTokens[req.remoteAddress] == null) {
...
const ipTokens = {
    "144.217.248.78": null, // Proxy
    "54.39.221.94": null,   // Hub/ Creative
    "51.222.102.72": null,  // Survival
    "51.81.182.151": null,  // Testing
}
...
api.get('/token', async (req, res) => {
    ...
    try {
        ...
        // Refresh token
            ...
            // Refresh on null is okay, tell server it needs to exchange
            if (tokenObj == null)
...
/**
 * Add, remove, or reset roles for a particular type
 *
 * {
 *     guild: String
 *     add?: String, String[]
 *     remove?: String, String[]
 *     reset?: "member", "moderator", "admin", "all"
 *     type?: "member", "moderator", "admin"
 * }
 *
 */
api.post('/discord/roles', async (req, res) => {
    if (!validateToken(req, res))
        return 0;

    let {guild, type, add, remove, reset} = req.body
    if (add)
        res.json(await database.discordRolesModify(guild, type, "add", add))
    else if (remove)
        res.json(await database.discordRolesModify(guild, type, "remove", remove))
    else if (reset)
        res.json(await database.discordRolesModify(guild, reset, "reset"))
    else
        res.json({error: 1, message: "Missing modification data! Must have one of 'add', 'remove', or 'reset'."})
})

/**
 * Get roles of the specified type for the discord guild
 *
 * {
 *     guild: String
 *     type?: "member", "moderator", "admin", "all"
 * }
 *
 */
api.post('/discord/roles/get', async (req, res) => {
    if (!validateToken(req, res))
        return 0;

    let {guild, type} = req.body
    res.json(await database.discordRolesGet(guild, type ?? "all"))
})
...
api.post('/discord/webhook', async (req, res) => {
    ...
    let {kind, name, avatar, message, guild} = req.body
    res.json(await discord.webhookSend(kind, name, avatar, message, guild))
})

/**
 * Retrieve webhooks on the guild
 *
 * {
 *     guild: String
 * }
 */
api.post('/discord/webhook/get', async (req, res) => {
    if (!validateToken(req, res))
        return 0;

    let {guild} = req.body
    res.json(await database.discordWebhookGet(guild, false))
})

/**
 * Create a discord webhook and add it to the database
 *
 * {
 *     name: String
 *     kind: String
 ...
 * }
 */
api.post('/discord/webhook/create', async (req, res) => {
    if (!validateToken(req, res))
        return 0;

    let {name, kind, channel} = req.body
    res.json(await discord.webhookCreate(name, kind, channel))
})

/**
 * Delete and remove a discord webhook
 *
 * {
 *     guild: String
 *     name: String
 ...
 * }
 */
api.post('/discord/webhook/delete', async (req, res) => {
    if (!validateToken(req, res))
        return 0;

    let {guild, name, channel} = req.body
    res.json(await discord.webhookDelete(guild, name, channel))
})
...
api.post('/log', async (req, res) => {
    let {message, exception} = req.body
    if (!validateToken(req, res))
        return 0;

    res.json(await database.logSave(message, exception))
})
